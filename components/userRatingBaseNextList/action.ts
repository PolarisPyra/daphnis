...
interface EnhancedRatingEntry extends RatingBaseNextEntry {
  chartId: string;
  title: string;
  artist: string;
  genre: string;
...
export async function getUserRatingBaseNextList(): Promise<EnhancedRatingEntry[]> {
  ...
  try {
    const userRatingBaseList = await fetchUserRatingBaseNextList(auth.user.UserId, supportedVersionNumber);
...
async function fetchUserRatingBaseNextList(
  userId: string,
  version: number
): Promise<RatingBaseNextEntry[]> {
...
function combineRatingWithStaticInfo(
  ratingList: RatingBaseNextEntry[],
