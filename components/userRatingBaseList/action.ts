...
interface EnhancedRatingEntry extends RatingBaseEntry {
  chartId: string;
  title: string;
  artist: string;
  genre: string;
...
export async function getUserRatingBaseList(): Promise<EnhancedRatingEntry[]> {
  ...
  try {
    const userRatingBaseList = await fetchUserRatingBase(auth.user.UserId, supportedVersionNumber);
...
async function fetchUserRatingBase(
  userId: string,
  version: number
): Promise<RatingBaseEntry[]> {
